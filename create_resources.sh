...
# Create snap-proxy-registration, snap-proxy and client instances
cat <<EOF > block.json
[
	{
                    "DeviceName": "/dev/sda1",
...
snapproxyreg_instance=$(aws ec2 run-instances --image-id $ami --instance-type $instance_type --key-name $keypair --security-group-id $security_group --subnet-id $subnet --block-device-mappings file://./block.json --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=snap-proxy-registration}]' 'ResourceType=volume,Tags=[{Key=Name,Value=snap-proxy-registration-vol}]' --region $region --query 'Instances[0].InstanceId' --output text)
...
snapproxyreg_public_ip=$(aws ec2 describe-instances --instance-ids $snapproxyreg_instance --region $region --query 'Reservations[0].Instances[0].PublicIpAddress' --output text)
...
wait_for_ssh $snapproxyreg_public_ip
wait_for_ssh $snapproxy_public_ip
wait_for_ssh $snapclient_public_ip

ssh $ssh_flags -i $ssh_key $snapproxyreg_public_ip "echo '$snapproxyip   snaps.canonical.internal' | sudo tee -a /etc/hosts"
